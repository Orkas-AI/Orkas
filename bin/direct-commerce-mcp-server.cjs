...
async function executeShopify(config, name, p) {
  ...
  if (name === 'refunds.create') return shopifyGraphql(config, 'mutation($input:RefundInput!,$idempotencyKey:String!){refundCreate(input:$input) @idempotent(key:$idempotencyKey){refund{id createdAt totalRefundedSet{shopMoney{amount currencyCode}}} userErrors{field message}}}', { input: p.input, idempotencyKey: randomUUID() });
