...
exports.shape = (z, bound) => {
  ...
  return {
    action: z.enum(['list', 'create', 'update', 'delete', 'enable', 'disable']).describe('create starts a new schedule; list first when the request may refer to an existing one. enable resumes scheduling; disable pauses it. Delete only when removal is requested.'),
    task_id: z.string().regex(/^at_[0-9a-f]{8}$/).optional().describe('Required for update/delete/enable/disable; use an id from list.'),
    ...
    enabled: z.boolean().optional().describe('Defaults to true on create; omitted updates preserve the current value.'),
    ...
    offset: z.number().int().min(0).optional().describe('List only; default 0. Continue with next_offset until null.'),
    limit: z.number().int().min(1).max(50).optional().describe('List only; default 20.'),
  };
};
