...
└── <user_id>/
    ...
    └── local/                 🔒 Machine-private domain (never synced)
        ├── config/            User prefs (auth-profiles / permissions / reflection-state / web-search-cache) — NEVER user-clearable
...
6. **Multi-device sync (plan: `PC/docs/plans/multi-device-sync.md`)**: COS bucket prefix = **OAuth `user_id`** (NOT the local 8-digit `uid`); per-machine path stays `<container>/data/<local_uid>/cloud/`, and `<oauth_user_id>/` is the cloud-side mirror. Why: same OAuth user has different local `uid` on each device — using local uid would scatter cloud data. The `features/sync/` engine is the **sole** writer of cloud→COS traffic and `<oauth_user_id>/cloud/*` keys; `features/sync/manifest.ts::commitManifest` (POST `/sync/manifest/commit`) is the **sole** writer of `<oauth_user_id>/manifest.json` (CAS + quota atomic on the Server side). PC's STS scope is `<oauth_user_id>/*` for read, `<oauth_user_id>/cloud/*` for write — `manifest.json` is read-only for PC by design.
...
- Add a new sync trigger pathway that doesn't go through the `DirtyBus` debounce. The five canonical triggers are: startup (post-marketplace reconcile), debounce-fired dirty event, manual ("立即同步" button), periodic 5-minute safety-net ticker (`engine.ts::PERIODIC_INTERVAL_MS`, bound to engine lifetime so the master switch gates it for free), before-quit. The 30s global floor between auto runs lives in `engine.ts::GLOBAL_FLOOR_MS`; manual + before-quit bypass it intentionally — periodic respects it (a recent pass for any reason defers the next tick).
