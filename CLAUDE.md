...
- main runs no HTTP / occupies no port / has no auth.
  - **Single documented exception**: `features/connectors/oauth.ts` binds an ephemeral 127.0.0.1 listener for the duration of one OAuth flow only (single `/callback` handler, auto-closes on success / timeout / quit). It's the standard desktop-OAuth pattern (Google Cloud SDK / gh / Notion CLI all do the same). It is **not** a general API surface — never extend this listener with other routes, never bind 0.0.0.0, never reuse it for non-OAuth purposes. Lifetime per flow is bounded by `FLOW_TIMEOUT_MS` (5 min); the call site auto-shuts the listener on completion. Any other feature that wants a localhost listener is forbidden.
...
- **MCP connector servers are a separate class with their own single spawn entry** — connector instances (see §6.5) spawn stdio MCP subprocesses via the `@modelcontextprotocol/sdk` `StdioClientTransport` inside `features/connectors/mcp-client.ts`, and **only there**. This is structurally analogous to the CLI-agent rule (one spawn entry per process class) but a separate code path: CLI agents are job runners producing one-shot answers; MCP servers are long-lived tool-providing daemons whose tools get folded into the in-process AgentRunner's `tools[]` array. Don't merge the two — they have different lifecycle, persistence, and cancellation rules.
...
- npm dependency allow-list is in `PC/package.json` (key entries: `electron / pi-ai / better-sqlite3 / sqlite-vec / fastembed / onnxruntime-node / pdfjs-dist / pdf-lib / mammoth / jimp / adm-zip / cos-nodejs-sdk-v5 / @modelcontextprotocol/sdk`). **New dependencies require a discussion first.**
...
**Platform-editing language constraint**: when the inline edit chat targets a platform-installed skill / agent (dev mode only — see §6), the system-prompt tail uses `i18n.ts::buildBuiltinEditingConstraint()` instead of `buildLanguageDirective()`. The directive forces English output for every field except `description_zh`, regardless of the user's UI language. **Why:** platform content ships to a multilingual user base; source files must stay English-only (see §6) and per-user UI language must not influence what gets published.

**Authoring rules live in platform skills, not in `chat_*.md`**: the canonical authoring rules for agents (`<agent>` container) and skills (`<skill>` container + `<<<skill-file>>>` blocks) live in two platform skills installed from the marketplace under ids `agent-creator` and `skill-creator`. The four prompts that previously copy-pasted these rules (`chat_commander.md`, `chat_agent_setup.md`, `chat_agent_setup_cli.md`, `chat_skill_setup.md`) now carry only a session-shell + `read_file <ROOT>/agent-creator/SKILL.md` (or skill-creator) pointer. **Why:** the rules are ~200 lines each and were duplicated across four prompts (~600 lines drift surface), kept in sync only by reviewer attention; the duplication caused the commander prompt to balloon past 460 lines, putting the rules in the lost-in-the-middle band. Moving them to platform skills cuts the always-loaded system prompt by ~590 lines and gives every authoring surface a single source of truth — the LLM `read_file`s the skill on demand. **When changing authoring semantics**: update `agent-creator/SKILL.md` or `skill-creator/SKILL.md` in your local install and republish through the marketplace upload flow; `grep -E "<agent>|<<<skill-file>>>|agent-creator|skill-creator" src/main/prompts/*.md` to confirm the four chat prompts still just point at the skill (no copy-back drift). The loader does not do template composition (only `$variable` substitution), so the on-demand read is the natural sync mechanism — every fresh authoring session re-reads the skill body.
...
└── <user_id>/
    ...
    └── local/                 🔒 Machine-private domain (never synced)
        ├── config/            User prefs (auth-profiles / permissions / reflection-state / web-search-cache / account = `(user_id, session_id)` + device_id + cached profile) — NEVER user-clearable
...
6. **Multi-device sync (plan: `PC/docs/plans/multi-device-sync.md`)**: COS bucket prefix = **OAuth `user_id`** (NOT the local 8-digit `uid`); per-machine path stays `<container>/data/<local_uid>/cloud/`, and `<oauth_user_id>/` is the cloud-side mirror. Why: same OAuth user has different local `uid` on each device — using local uid would scatter cloud data. The `features/sync/` engine is the **sole** writer of cloud→COS traffic and `<oauth_user_id>/cloud/*` keys; `features/sync/manifest.ts::commitManifest` (POST `/sync/manifest/commit`) is the **sole** writer of `<oauth_user_id>/manifest.json` (Server enforces atomicity via per-user lock + generation CAS + quota delta; see `Server/CLAUDE.md` §4). PC's STS scope is `<oauth_user_id>/*` for read, `<oauth_user_id>/cloud/*` for write — `manifest.json` is read-only for PC by design. **Five canonical sync triggers** funnel through `DirtyBus` / `engine.syncNow`: startup (post-marketplace reconcile), debounce-fired dirty event, manual ("立即同步" button), 5-minute periodic safety-net ticker (`engine.ts::PERIODIC_INTERVAL_MS`, bound to engine lifetime so the master switch gates it for free), before-quit. The 30s global floor between auto runs (`engine.ts::GLOBAL_FLOOR_MS`) applies to all except manual + before-quit (which bypass it intentionally; periodic respects it — a recent pass for any reason defers the next tick).
...
**iOS remote-control relay** — `features/relay/` (Server-dependent, **stripped from the OrkasOpen build** like `features/account/`). The companion `iOS/` app is a remote control: it can't run agents, it dispatches commands to this PC and watches progress. While the account is signed in (wired from `index.ts` after `account.bootstrap()` resolves to `ready`): `device.ts` registers this PC as a relay device + heartbeats (`device_id` reuses `account/token_store.ts::getDeviceId` — a stable per-machine UUID persisted in `account.json`); HTTP / WS calls attach `user_id` + `session_id` from `token_store.authHeaders()` (the unified SessionMgr credential — no separate access/refresh token layer); `commands.ts` long-polls `/relay/commands/poll` for commands the iOS app addressed to this device and delivers each as a user message via `group_chat.send` (→ `bus.enqueue`, the single dispatch primitive — **no parallel dispatch path**); `sync.ts` mirrors a `state.json::relay_enabled` conversation's `message` / `plan_changed` / `state_changed` bus events up to `/relay/conversations/sync` so the phone sees the same messages + plan rail. A relayed command's cid is the one the Server assigned (a fresh `rconv-…` for a new conversation, or the phone's existing cid); `chats.createConversation`'s `conversationId` option adopts it so both ends agree on the cid. **No server-side agent execution** — the Server `relay` module is a thin presence + queue + conversation-mirror layer (see Server CLAUDE.md); a server-side "云端" worker is a future decision, and the iOS device picker shows "云端" permanently greyed until then.
...
Third-party tool integrations attach via MCP servers (`@modelcontextprotocol/sdk`); Orkas is purely an MCP **client**, and **every installable connector uses OAuth 2.0** — no API-key / PAT entry point in the user-facing surface. We don't host first-party MCP servers; the chosen community MCP server is spawned locally (stdio) or reached remotely (streamable-http) with the user's OAuth `access_token` injected per the catalog entry's transport template at every spawn.

**Curated catalog**: every installable connector is a row in `features/connectors/catalog.ts::CONNECTOR_CATALOG`. Each entry declares `oauth` (the per-provider OAuth 2.0 endpoints + default scopes + PKCE / secret flags) and `transport_template` (the MCP server we spawn, with either `oauth_env_key` or `env_synthesizer` describing how the access_token reaches the server). UI renders one card per catalog row. **Adding a new connector** = append to the catalog (set `transport_template: null` + `unavailable_reason: 'oauth_pending'` when the MCP server target isn't selected yet; otherwise wire `oauth_env_key` / `env_synthesizer`). `manager.connectViaOAuth` is the only public install path — there is no free-form / API-key entry.

**OAuth flow**: PKCE + ephemeral 127.0.0.1 callback listener (the §1 single-spawn exception). `features/connectors/oauth.ts::startOAuth` runs the full flow, returns an `OAuthGrant`. `refreshIfStale` runs lazily at boot / `connectors.refresh` / on the next reconnect — no background timer. Mid-call expiry surfaces as a tool error; the user re-clicks "刷新工具" and the manager re-resolves the transport with a freshly-refreshed token.

**Provider client credentials**: `client_id` / `client_secret` come from `features/connectors/oauth-config.ts`, two sources in priority order:
1. Env vars `ORKAS_OAUTH_<PROVIDER>_CLIENT_ID` / `..._SECRET` — baked into Hosted Orkas builds.
2. `<uid>/local/config/connector-oauth.json` (BYO mode for OrkasOpen / power users who registered their own apps).

A catalog entry without resolvable credentials renders disabled in the gallery (`需先配置 OAuth`). The OrkasOpen build inherits the catalog and the BYO flow as-is; nothing in `features/connectors/` is stripped — connectors are still 100% client-side.

**Storage**: single file `<uid>/local/config/connectors.json` (path helper: `paths.ts::userConnectorsConfigFile`). Encrypted via `util/crypto-vault.ts` (PBKDF2 → AES-256-GCM, key derived from the active uid + a compile-time app salt). `connector-oauth.json` uses the same vault. **The encryption is obfuscation-grade**, not keychain-strength — see `util/crypto-vault.ts` header. The product owner accepted that trade-off explicitly to keep everything portable inside the data directory (no OS-keystore dependency). The same vault wraps `auth-profiles.json` (LLM provider credentials) since those are credentials of equal sensitivity.

**Lifecycle**: `features/connectors/manager.ts` is the process singleton. `bootstrap(uid)` reads the registry and reconnects everything in parallel at boot; `before-quit` calls `shutdownAll()` so stdio subprocesses (the MCP servers) exit instead of becoming zombies. Status flips on each turn (`connected | connecting | disconnected | error`) are persisted but treated as best-effort — the live `Map<id, McpConnection>` is the truth at runtime.

**Tool injection (umbrella meta-tools + system-prompt block)**: connectors are exposed through (1) a `## Connectors` markdown block injected into the system prompt enumerating each visible connector (`- **<id>** — <display_name>: <description> [(account: <label>)] [— <status>]`; status suffix appears only when the instance is not `connected`; account suffix only when the OAuth grant carries `account_label`; description pulled from `catalog.ts::CONNECTOR_CATALOG[*].description_{zh,en}` in the user's UI language), plus (2) two meta-tools — `list_connector_tools(connector_id)` (returns the JSON schemas for one connector's actions) and `call_connector_tool(connector_id, tool_name, args)` (routes to `manager.callTool`). Both come from `model/core-agent/connector-meta-tools.ts` (`getConnectorPromptBlock` + `createConnectorMetaTools`). The block carries no protocol-teaching paragraph — each per-role chat prompt teaches its own usage (commander / agent-worker → invoke; agent-edit → reference by id in workflow). Per-connector MCP actions are NEVER injected into `tools[]` — the model reads the catalog from the system-prompt block (no discovery round-trip), gets per-action schemas from `list_connector_tools`, and invokes through `call_connector_tool`. The visibility resolver `features/connectors/tools-adapter.ts::resolveVisibleConnectors` is shared by both meta-tools and the prompt block so the actor matrix (commander vs. agent worker / `enabled_connectors` whitelist / instance-level `enabled_subtools` filter) is single-sourced. **The block + both tools are omitted entirely when zero connectors are visible to the actor** — `tools[]` shrinks by two slots, the system prompt stays smaller, and the model has no broken affordance to call.

**Session-kind gate (tri-state)** (`runner.ts::_connectorExposure`):
  - `gconv` (commander) + `gmember` (agent worker) → **block + tools** (full exposure: read catalog AND invoke).
  - `agent` (agent-edit chat) → **block only** (the editor LLM authoring the agent's `<workflow>` needs to know which connectors exist so it can write "use the `notion` connector to …" or similar; the meta-tools themselves are NOT injected — an authoring session must not produce external side effects). The block in this mode bypasses `agent.enabled_connectors` (passes `undefined` agentId to `getConnectorPromptBlock`) — the editor LLM must see EVERY installed connector even if the agent's current allowed-list hasn't been opened to it yet, otherwise it can't recommend referencing one (chicken-and-egg: the user toggles `enabled_connectors` separately in the agent-edit UI's "Allowed connectors" section, not via the LLM).
  - `skill` (skill-edit) / KB-image / CLI dispatch / reflect / memory-extract / anon → **none** (no block, no tools — these aren't user-task surfaces and shouldn't fire external side effects). Adding a new conversation kind that needs connectors? Extend `_connectorExposure` explicitly + add an entry to §5's session-id table.

**Why the umbrella architecture**: keeps `tools[]` size constant regardless of connector count, avoiding the 20–50 tool-selection-accuracy cliff every major LLM hit in 2025–2026 (Anthropic 30–50 / Cursor ~40 / Cline ~20 / Copilot 128 hard cap with degradation well below); preserves the Anthropic prompt-cache prefix (`tools → system → messages`) across connect/disconnect events (the block is part of the appended system prompt, stable per session); provider-agnostic under the pi-ai abstraction (no dependency on Anthropic's private Tool Search Tool). Static `TOOL_CATALOG` (§3) enumerates the two meta-tools but **not** the dynamic per-connector actions (they vary per-user). Tool name shape `<instance_id>__<mcp_tool_name>` is retained for internal routing + the dev archive but no longer surfaces to the model.

**Per-agent scope**: `agent.enabled_connectors` is a whitelist of instance ids. **Semantics intentionally diverge from `agent.skill_list`'s three-state**: missing/`undefined` and `[]` BOTH collapse to "no connectors"; only a non-empty `string[]` grants access. **Why:** connectors carry external side effects (sending emails, mutating remote docs) — a brand-new agent must opt in explicitly, while skills are passive instructions where "no filter" is a reasonable default. The Commander (agentId === undefined at the runner level) sees every connected instance — it's the routing layer and needs full visibility. The field is authored exclusively in the agent edit UI's "Allowed connectors" section, not by the agent-edit LLM (the `<agent>` container has no sub-tag for it). Under the umbrella architecture, the whitelist controls what appears in the `## Connectors` system-prompt block at runtime (gmember sessions); when the resulting visible set is empty, the block AND both meta-tools are omitted from the runner output. **Agent-edit sessions deliberately ignore the whitelist** (see the tri-state gate above) so the editor LLM can recommend a connector the agent hasn't been opened to yet.

**Spawn discipline**: §1 calls out `features/connectors/mcp-client.ts` as the sole spawn entry for MCP stdio subprocesses, and `features/connectors/oauth.ts` as the sole localhost-listener exception. Other modules use `manager.callTool` / `manager.connectViaOAuth` — no direct `child_process` / transport / HTTP listener outside these two files.

**OrkasOpen**: connectors have no Server dependency, so the entire `features/connectors/` tree + the renderer's `panel-connectors` + locale keys + `enabled_connectors` field stay in the OrkasOpen build. (Marketplace-style recommended-connectors UI, if added later, can be conditionally hidden the same way `marketplace_dev` is — see §9 OrkasOpen contract.)
...
- **No fallback strings on `t()` calls** — never write `t('foo.bar') || 'Fallback…'`. Locale tables are the single source of truth; a missing key is a bug, not a "graceful degradation" condition. **Why:** fallback strings drift away from the locale tables (the tables stop being authoritative), they leak source text into a language the file is supposed to be neutral toward (Chinese fallbacks in source violate the project's English-only-source rule), and they hide missing-key bugs that would otherwise surface immediately. **The rule:** add the key to both `zh.json` and `en.json` before the code can ship; if a key is missing the raw key string surfaces in UI (per `t()`'s built-in raw-key fallback) and the dev fixes it. Same rule applies to `t()` arguments that are interpolated into HTML — write the key, not a `||` chain.
...
- Write `<oauth_user_id>/manifest.json` from PC (any code path). The Server's `/sync/manifest/commit` is the **sole** writer; the CAS + size-delta quota check is server-side atomic. PC has STS scope `<oauth_user_id>/cloud/*` for writes — `manifest.json` is read-only for PC by design.
...
- Bypass `features/sync/transport.ts` for any object under `<oauth_user_id>/`. Anyone touching the sync engine's bucket must go through that module (STS caching, gzip whitelist, multipart upload, rename optimization all live there).
...
- Persist connector tokens to `<uid>/cloud/` — they're machine-private, MUST stay under `<uid>/local/config/connectors.json` (sync would propagate bearer material across devices the user didn't intend).
- Write `connectors.json` / `connector-oauth.json` / `auth-profiles.json` as plaintext — every credential-bearing file under `<uid>/local/config/` goes through `util/crypto-vault.ts`. The legacy plaintext-read path is a one-shot migration affordance, NOT a fallback; never special-case a code path that emits plaintext.
...
- Expose connector meta-tools to a non-group-chat session — only `gconv` (commander) + `gmember` (agent worker) get them. `agent` (agent-edit) gets the `## Connectors` block ONLY (no tools — authoring sessions must not produce external side effects); `skill` / KB-image / CLI / reflect / memory-extract / anon get neither block nor tools. The single source is `runner.ts::_connectorExposure` (per §6.5 tri-state gate). Adding a new conversation kind that needs connector access requires extending this function explicitly + an entry in §5's session-id table.
