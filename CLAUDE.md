...
2. **`data/builtin/{agents,skills}/` is a runtime copy**: synced by hash from `src/builtin/` at startup. Agents are directories (`<aid>/agent.json`); skills are directories (`<sid>/SKILL.md`). The loader scans `[<uid>/cloud/, data/builtin/]`; custom takes precedence over a builtin with the same id. `kind` is decided by the spec's root (there is **no** `custom/` / `builtin/` layer inside the directory). **Marketplace exemption**: dirs that carry a top-level `_marketplace.json` sentinel are out-of-band installs from the marketplace (see §6); `syncBuiltin{Agents,Skills}` excludes them from the hash and from the stale-removal pass, and a same-id `src/builtin/` ship does NOT overwrite a marketplace install (the sentinel signals a user-driven choice). The sentinel file itself is never injected into the SkillLoader prompt or read by the agent loader — it's metadata for the sync layer only.
...
Sources = `src/builtin/skills/` (git-tracked, hash-synced into `data/builtin/skills/` at startup) + `<uid>/cloud/skills/` + **marketplace installs** (also land under `data/builtin/skills/<id>/` but with a `_marketplace.json` sentinel — see `features/marketplace.ts`; the sentinel is what makes the startup sync leave them alone, per §4 constraint 2). `SkillLoader` scans `[user, builtin]` and injects them into the system prompt; **custom takes precedence over a builtin with the same id**. Built-ins are read-only in packaged builds. The same marketplace mechanism applies to agents (`data/builtin/agents/<id>/agent.json` + `_marketplace.json`); the on-disk spec shape is unchanged — `category` / `version` live in the sentinel only, never in `agent.json` / SKILL.md frontmatter.
