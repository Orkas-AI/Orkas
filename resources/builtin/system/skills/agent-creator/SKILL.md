...
3. **Preserve action authority across irreversible operations.** A current user request authorizes that exact action. For steps that delete, overwrite, or mutate external state, distinguish an unresolved target from a scope expansion: ask only for the missing target, and add a pause only for a materially different action/target or a platform-required confirmation. Recoverable in-workflow exception handling stays out (handled by the runtime — existing rule).
...
2. **Confirm the agent is editable here.**
   ...
   - `runtime.kind === 'cli'` (external CLI agent: claude code / codex / openclaw / opencode / hermes — they bring their own prompt; the selected CLI plus optional per-Agent model/thinking overrides are owned by the create modal + detail form, and unset overrides stay in that CLI's account/configuration) → in the per-agent CLI edit chat, only `name` / `description` (or explicit localized descriptions) / `inputs` / `interactive` are editable; do NOT emit `<workflow>` / `<knowhow>` / `<standards>` / `<skills>` / `<runtime>` / `<system>` / `<persona>`. In the group-chat commander, reply with one prose line saying these are detail-panel-only and stop.
...
- **NOT editable from any LLM surface**: `workflow` / `knowhow` / `standards` / `skills` / `runtime` / `system` / `persona`. The runtime (which CLI, optional per-Agent model/thinking overrides, and extra args) is configured by the user via the modal + detail UI; unset overrides keep the CLI default and never rewrite global CLI configuration.
