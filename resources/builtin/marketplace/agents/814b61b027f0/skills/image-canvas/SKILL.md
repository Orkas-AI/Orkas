...
Keep every `may_change` list within the changes the user explicitly allowed. An axis that a reference does not control stays unbound; it does not become permitted to change. For example, when a composition reference may change only to fit an aspect ratio, list only the necessary crop, framing, or layout adaptation—not scene, subject, content, or background changes.
