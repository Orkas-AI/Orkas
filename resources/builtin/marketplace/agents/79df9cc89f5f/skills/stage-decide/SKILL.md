...
3. **Record the edit strategy and references.** Write `plan.json::edit_strategy` with deterministic/mixed mode, concrete objectives, only the transcript/scene/silence/quality/vision signals actually used, and non-overlapping preserve/may-change boundaries. Record every source or guiding image/video in top-level `references`; video timing or motion guidance needs temporal anchors.
