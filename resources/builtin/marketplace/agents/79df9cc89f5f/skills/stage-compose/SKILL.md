...
## Post-approval amendment de-duplication

A user-requested edit after Gate B, Preview, or Draft must produce **at most one approval form for one bounded change**. Do not make the user approve a visual recovery and then approve the same wording/layout change again at Gate B.

Before opening any post-approval form, call `composition.status` and classify both the native state and the requested patch:

1. **Visual-only patch:** HTML/CSS/SVG/layout/motion changes that do not change `script.md`, `shotlist.json`, or the manifest's canonical Gate B payload (`composition` delivery fields, scene `approved_copy`, `narration_text`, `narration_refs`, `source_shots`, `roles`, or `audio.narration_intent`). Continue the existing repair cycle and edit directly. Do not call `composition.begin_visual_revision` and do not ask for recovery authorization unless the latest native result itself returned `visual_revision_recovery_available:true`.
2. **Gate B payload patch:** casing, punctuation, wording, language, timing, narration, source mapping, roles, or delivery changes that alter the signed payload. Present one amendment summary and one form containing the canonical `gate_b_decision` field. If clarification is needed, put the clarification fields in this same form and label approval as applying **and signing** the selected bounded amendment. Do not first collect the same choice in a separate form and then ask for a technical re-sign.
3. **Gate B payload patch plus exhausted visual QA:** use one combined form containing both `gate_b_decision=approve|revise` and `visual_recovery_decision=new_visual_revision|pause`, plus any bounded adjustment fields. One submission authorizes the whole stated patch; never emit two sequential forms for it.

When the native HTML Preview Gate will still be required, amendment/Gate B copy must not promise that this approval immediately re-renders the video. Say exactly what the turn authorizes: apply and sign the bounded amendment, run native QA, and generate a fresh reviewable preview. Also state that a passing preview is shown in one later HTML Preview Gate before rendering. The preview gate reviews a newly generated artifact; it is not another approval of the same script or amendment.

In the submitted combined/amendment turn, perform operations in this order:

1. Call `composition.begin_visual_revision` only when that same form included `visual_recovery_decision=new_visual_revision` **and** the immediately preceding native result reported recovery available.
2. Apply exactly the amendment described by the form response, synchronizing script/shotlist/manifest/HTML only where the selected change requires it.
3. If the Gate B payload changed, call `composition.approve_plan` **after** the files are updated, while the same real user turn still carries `gate_b_decision=approve`.
4. Then call `composition.doctor` or `composition.reconcile` as required, followed by lint/inspect/snapshot. Draft only when no Preview Gate is required or after a later explicit preview approval. Never call lint before re-signing a changed Gate B payload, because the predictable `E_GATE_B_ARTIFACT_CHANGED` result must not be converted into another user confirmation.

If snapshot creates a fresh preview, show that artifact and emit exactly one canonical `preview_decision` form, titled and described as the **only remaining confirmation before render**, then stop. Native preview approval must come from a later real user turn, so never imply that the earlier Gate B or visual-recovery approval can approve an unseen future preview. After a passing snapshot, do not reopen visual recovery or Gate B unless the user selects `revise` and requests a new artifact-changing amendment.

For an in-flight conversation that already entered the old split-form sequence, converge instead of alternating: after `composition.begin_visual_revision` succeeds, finish the bounded patch and open Gate B at most once if its signed payload changed; after `composition.approve_plan` succeeds, run QA and advance only to the single Preview Gate. Never ask for visual recovery again merely because Gate B was signed in a later turn.

`composition.submit_design_review` returning `status:"repair"` with `next_action:"repair_visuals_then_composition.reconcile"` is not visual-cycle exhaustion. Follow that next action in the current cycle. Never call `composition.begin_visual_revision` merely because a draft design review requested a localized repair.

If a form submission already contained `gate_b_decision=approve` for the exact bounded patch that was then applied, `E_GATE_B_ARTIFACT_CHANGED` means the agent missed the required same-turn `composition.approve_plan`; call it in that turn when still possible, rather than asking the user to approve the identical patch again.
