...
- **Exhausted visual QA cycle:** pass the exact native result to `gate-control` and obey its resolver. This skill does not decide whether a form or `composition.begin_visual_revision` is allowed.
...
After the resolver authorizes a normal revision, edit the bounded files, call `composition.reconcile`, and continue inspect/snapshot. Call `composition.begin_visual_revision` only when the resolver returns `next_action:"begin_visual_revision"`; ordinary Preview/Gate D feedback, design-review repair, `E_VISUAL_REVISION_NOT_REQUIRED`, and remaining repair passes never do.
