...
How to execute a validated `project/plan.json` into one finished file. By the time you are here the plan passed the `stage-plan video_plan --op validate` script and `video_studio production.approve_plan` recorded explicit Gate B approval. Start/resume with `production.status`; if `plan_approval_current` is false, stop instead of producing or opening child gates. Walk the signed EDL; do not re-plan. Host-neutral: VideoStudio-specific edit/plan work runs through skill scripts (`stage-edit edit_video`, `stage-plan video_plan` via `bin/run-skill.cjs`), while production approvals, composition, and transcription run through the required built-in `video_studio` runtime; generic built-in capabilities remain `generate_video` / `generate_image` / `generate_speech`.
...
- **compose** → `stage-compose`: under `project/compositions/<id>/`, write local `script.md`, `shotlist.json`, and `composition-manifest.json` derived exactly from the signed segment's `spec.composition_plan`. Keep it silent (`audio.owner:"none"`, no tracks). Call `video_studio` `composition.approve_plan` with `composition_dir`, `plan_path:"project/plan.json"`, and this `segment_id`; this verifies parent Gate B and records inherited child approval without another user Gate B. Then doctor → prepare → visual authoring → inspect → any native-required snapshot/preview → draft to `project/parts/<id>.mp4`. A binding mismatch returns to the single parent EDL Gate B; never invent a child approval form or call draft directly.
- **generate** → `stage-generate` (+ `stage-consistency` for recurring characters): only AFTER `production.status` reports the current Gate C signature. Call `generate_video` or `generate_image` with `production_plan_path:"project/plan.json"` and `production_segment_id:<id>` → `project/assets/<id>.<ext>`. Every auxiliary portrait/keyframe is already its own signed generate segment; do not create unplanned billable calls. The host transaction reuses a completed artifact and blocks an interrupted/failed duplicate. `production.status` reports local durable state; it cannot query an uncertain provider transaction. Never automatically retry under the same Gate C approval or invent a recovery API. Report pending/failed state and, only if the user wants another paid attempt, open a fresh Gate C; an uncertain pending attempt must use a new output path.
