...
name: stage-plan
description_zh: "摄入用户素材并把端到端视频需求规划成一份经校验的跨模态 EDL，覆盖剪辑、生成、合成、已提供片段以及旁白、音乐、字幕和交付承诺；用于 AUTO 制作审批前的计划阶段，不负责最终成片装配。"
description_en: "Ingest user media and turn an end-to-end video request into one validated cross-modal EDL covering edit, generate, compose, provided segments, narration, music, captions, and delivery promises. Use for AUTO planning before approval, not final-video assembly."
---
...
- **source_led** — the user's footage is the hero (repurpose / highlight / localize / semantic edit). `source_required: true`.
...
- `source`: **edit** (trim a real clip — needs `input_id` + `in_sec`/`out_sec`), **generate** (billable AI media — needs `prompt`, explicit `media_kind:image|video`, and the exact provider fields the call will use, written flat on `spec` — `operation`, `generation_duration_sec`, `resolution`, `quality`, `generate_audio`, plus every reference path/URL. A nested `spec.settings` object is not read by anything: the validator sees no provider fields and the host executes its own defaults. For video the approved defaults are plan aspect, clamped 4-15s target duration, 720p, balanced, audio on), **compose** (designed HTML — needs `kind` plus a complete `composition_plan.scenes` binding with each scene's id, approved_copy, narration_text, and semantic roles), **provided** (use a supplied asset as-is — needs `asset_id` and mandatory `kind:image|video`; unknown kind counts as neither footage nor motion).
- `layer`: **primary** (the main timeline), **overlay** (sits over a primary via `over: <segment id>` — captions, lower-thirds, logo boxes), **bg** (behind). An overlay composited onto real footage must be SMALLER than the frame: the pipeline renders composed overlays as opaque video without an alpha channel, so a full-frame overlay does not blend — it replaces every pixel and deletes the footage under it (the assembler refuses this as `E_EDIT_OVERLAY_OPAQUE`). For a full-frame branded look on top of a real clip, plan edge elements (corner logo, lower-third strip) as the overlay, or make that beat a composed `primary` segment with no footage underneath.
...
3. Gate B for a production EDL is two turns, not a state to re-check. In the presenting turn: call the native production status operation — for an unapproved plan it returns `plan_summary`, the host's own rendering — present that summary verbatim in the user's language, invite changes in one line, and end the turn. In the NEXT turn, the user's reply is the decision: hand it to `gate-control` and perform the Gate B transition it returns. Never present the summary twice for the same plan; an unapproved plan after the user already answered means you owe the transition, not another confirmation. Never ask for approval of a plan you have not shown — a locked-facts line plus the per-segment timeline with its copy is the minimum. Gate B is the highest-leverage checkpoint: it is far cheaper to fix the plan here than after assembly.
4. Hand the displayed EDL, locked line, and production artifact state to `gate-control`, then perform only its returned Gate B transition. This project-scoped signature is the approval source for EDIT/AUTO/GENERATE and every AUTO child composition; never infer approval from a turn boundary.
5. When the signed plan contains generate segments, call `production.status` and supply `gate-control` with the exact `cost_estimate.billable_generations` count, configured external provider, external-billing disclosure, and current transaction state. State that the open build cannot verify the provider's billing or balance locally, and do not present an unavailable provider as review-ready. Every billable tool call must carry the approved plan path plus its own segment id. A plan edit invalidates both Gate B and Gate C authorization.
