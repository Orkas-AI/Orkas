...
4. If multiple inspectable source screens define the scope, enumerate the complete set and keep one row per source: identity/path, target route/component, must-preserve visible anchors, intentional changes, and separate `inspected`, `implemented`, and `compared` status. Do not sample the set when promising complete coverage.
5. Implement in small changes.
6. Check text fit at the actual requested viewport; check responsive behavior only when the user requests multi-device or narrow-screen support.
7. For a standalone artifact, include visible focus treatment, semantic labels or alt text, and `prefers-reduced-motion`; keep every requested section and action reachable, and back each action with included or inline sample content rather than a missing placeholder file.
8. For a runnable local HTML entry, call `html_preview` with the actual requested viewport target. `screenshots` defaults to false; use `screenshots:true` only for the final visual review. Use `target:"responsive"` only for an explicit responsive, multi-device, or narrow-screen request; otherwise omit target for desktop or use mobile for an explicitly mobile artifact. Source inspection, a media-query grep, a local server, PID, or HTTP response cannot prove rendered behavior.
9. If any requested viewport fails, repair the reported defect and rerun the same entry; failed checks return diagnostics without screenshots. Do not mask horizontal overflow with a global `overflow-x: hidden` or shrink the entire page.
10. Before claiming a multi-screen source set complete, compare every promised screen against a fresh rendered target. Build success, route/link validity, shared tokens, no-overflow checks, and spot checks do not prove unreviewed screens. If coverage is partial, name every remaining source; if the user contradicts fidelity, reopen the full ledger before asking for an example already present in the sources.
11. In the final handoff, name every user-requested screenshot size actually captured and inspected. Report Tab-key focus traversal and visible-focus counts, hash/mail links, filled/submitted forms, and observed download filenames, MIME types, and byte sizes.
12. When `html_preview` is unavailable or fails for an environmental reason, label browser, requested viewport behavior, link, form, and download checks unverified and provide copy-paste deployment steps for a static host. Still make the verification plan concrete: name each requested viewport, overflow/text-fit checks, tab order and focus, link targets, form behavior, and generated download content and filename. Do not search for or install another browser runtime.
