 ...
 *
 * Includes:
 *   - `google-workspace` (UI-only bundle; `bundle_member_ids` fans out OAuth to all 5)
 *   - `gmail`, `gcal`, `gdocs`, `gsheets`, `gtasks` (the 5 member entries, each backed by a
 *     local stdio adapter under `PC/bin/`)
 *
 * Adding a new Google service: drop a new entry here + `bin/<svc>-mcp-server.cjs` adapter +
 * Server's `_SCOPES_BY_CATALOG_ID` row, and union the scope into `google-workspace`'s
 * member list if you want it covered by the bundle's single consent.
 */
import type { CatalogEntry } from './types';

export const GOOGLE_ENTRIES: CatalogEntry[] = [
  // ── Google Workspace ────────────────────────────────────────────────────
  // One UI bundle (`google-workspace`) groups the 5 service entries below — a single OAuth
  // flow with `union(scopes)` provisions all 5 instances. Each member entry keeps its own
  // stdio adapter / tool surface so the model still sees 5 independent connectors with
  // service-specific tool names. Bypasses Google's MCP wrapper service
  ...
  // rationale and PC/CLAUDE.md §6.5.
  {
    ...
    description_zh: 'Gmail / Google 日历 / Google 文档 / Google 表格 / Google 任务 — 一次授权,5 项服务一起接入。',
    description_en: 'Gmail / Google Calendar / Google Docs / Google Sheets / Google Tasks — one consent, all 5 services connected.',
    auth_mode: 'server_bridge',
    oauth: { provider_id: 'google' },
    // Bundle: no transport of its own; OAuth fans out to the 5 member instances below.
    transport_template: null,
    bundle_member_ids: ['gmail', 'gcal', 'gdocs', 'gsheets', 'gtasks'],
  },
  {
    id: 'gmail',
    display_name: 'Gmail',
