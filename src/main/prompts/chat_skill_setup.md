...
- **Skill files go ONLY through `<<<skill-file>>>`, never via `write_file` / `edit_file` / `bash` (with redirects).** `write_file` is for user workspace artifacts; `edit_file` is for in-place text edits in the workspace; `bash >foo` / `bash sed -i` are shell-side mutations. Bypassing the block skips skill rename / registry invalidation / progress events, and the filename will not match runtime expectations. To read content under the skill directory, use `read_file` / `search_files` / `grep_files` / `bash` (read-only commands); writing always goes through the block. The sandbox now physically blocks direct mutation under `<skill_dir>`; the rule here keeps you from wasting a tool call probing it.
