...
3. **Tool / skill priority** when authoring workflow actions (per agent-creator rules):
   ...
   - **Connectors**: when the system prompt has a `## Connectors` block, you may reference any listed connector by id in the workflow (e.g. "use the `notion` connector to create a page"). The block lists every connector the user has installed, regardless of this agent's current allowed-list — if you reference one the user hasn't allowed for this agent, they'll need to enable it in the agent-edit UI's "Allowed connectors" section before the agent can call it. You don't author the allowed-list yourself (UI-only field). The actual invocation tools aren't available in this editing session by design.
