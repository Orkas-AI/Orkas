 ...
 *   reflect | anon | cli}. The kind keyword is the FIRST
 ...
 * sid / aid).
 */
const EPHEMERAL_KINDS = ['extract-img', 'reflect', 'anon', 'gworker'] as const;
 ...
 *  `extract` + leftover. */
const KNOWN_KINDS_RE = /^(gmember|gworker|gconv|extract-img|reflect|skill|agent|anon|cli)(?:-|$)/;
 ...
 *  one-shots) → null (no memory read or write). When null the runner
...
export function resolveSessionPath(userId: string, sessionId: string): string {
  if (!KNOWN_KINDS_RE.test(sessionId)) {
    throw new Error(
      ...
      `(gconv | gmember | gworker | skill | agent | extract-img | reflect | anon | cli)`,
    );
  }
