...
| Lark incremental consent | Request old grants plus missing user scopes; cancellation and absent grants remain failures. Verified user consent succeeds even with bot/resource advisories or legacy unresolved state; preserve those advisories and newer denials, without replay. A successful scope check clears only proven scope deficiencies. | `local-cli-files.test.ts` |
...
| Card action and recovery | Use stays available. Missing user scopes offer Reauthorize; Feishu/Lark business-only advisories show localized configuration/resource guidance without OAuth. Mixed cases show both. Unknown identifiers render as text, and only verified scope recovery removes the scope notice. Preserve selected tools. | `connectors-degraded-card.test.ts`, `manager.test.ts` |
