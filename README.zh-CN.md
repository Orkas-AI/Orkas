...
- **用自己的会员或 API key 接模型**——OAuth 登录会员账号或填 API key，接入 DeepSeek · Kimi · GLM · MiniMax · 豆包 · Qwen · Claude · OpenAI · Gemini 等，每个对话单独配；请求 Orkas 不经手、不存档
- **跨平台**——macOS（Apple Silicon + Intel）和 Windows 一等支持，Linux 社区级
...
去 [GitHub Releases](https://github.com/Orkas-AI/Orkas/releases) 找最新 tag，下载对应平台的安装包：

- **macOS** —— `.dmg`（Apple Silicon + Intel 通用）
- **Windows** —— `.exe`（NSIS 安装器）

装完打开应用，**设置 → AI 模型供应商** 配置 API key 或 OAuth 即可。
...
首次启动会在 `~/.orkas/`（macOS / Linux）或 `<字母最小的非系统盘>:\.orkas\`（Windows）创建工作目录。

---

## 致谢

Orkas 的部分核心机制参考自以下开源项目，特别致谢：

- **[OpenClaw](https://github.com/openclaw/openclaw)** —— `src/core-agent/` 是 OpenClaw 核心模块的精简提取；会话管理（`PersistentSession`）、运行循环（`AgentRunner`）、内存索引层均借鉴其设计
- **[Hermes-Agent](https://github.com/NousResearch/hermes-agent)** —— `skill_manage` 工具的设计参考自其 `skill_manager_tool`，让智能体可以在运行中创建 / 编辑自己的私有技能
