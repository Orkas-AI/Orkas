...
describe('preview source opt-in and revision isolation', () => {
  it('preserves ordinary HTML and fails closed for invalid or unsupported opt-ins', () => {
    expect(previewBundle(entry)).toBeNull();
    for (const text of ['{', '{"sdkVersion":2,"capabilities":[]}', ' '.repeat(16385)]) {
      manifest(text); expect(() => previewBundle(entry)).toThrow('E_MANIFEST');
    }
  });
  it('serves only the frozen bundle and invalidates calls after loaded source changes', () => {
    manifest(); const bundle = previewBundle(entry)!;
  ...
  it('rejects linked manifests, sibling files and linked assets before launching', () => {
    ...
    expect(() => previewBundle(entry)).toThrow('E_MANIFEST');
    ...
    expect(() => previewBundle(entry)).toThrow('E_BUNDLE');
  });
});
...
it('accepts only its own trusted frame, token and narrow operations, then revokes resources and deletes storage', async () => {
  ...
  const preview = await createPreviewSdk(ses, contents, previewBundle(entry)!);
  try {
    ...
    expect((await resource!(new Request(preview.origin + '/__orkas/sdk.js'))).status).toBe(403);
