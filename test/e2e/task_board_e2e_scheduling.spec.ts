...
test.describe('task board scheduling (D18 serial chain)', () => {
  ...
  test('edits through the composer, holds admission, and executes the saved multiline message', async ({ modelOrkas, boardPage }, testInfo) => {
    const app = modelOrkas;
    const page = boardPage;
    const before = app.modelRequests.length;
    const input = page.locator('#chat-input');
    ...
    app.setModelMode('controlled-slow');
    await input.fill('@ContentWriter keep writing the first draft.');
    await input.press('Enter');
    await expect.poll(() => app.modelRequests.length).toBe(before + 1);
    await input.fill('@ContentWriter old pending wording.');
    await input.press('Enter');
    const oldRow = page.locator('#chat-task-board-list .chat-queue-item', { hasText: 'old pending wording.' });
    await expect(oldRow).toBeVisible();
    const taskId = await oldRow.getAttribute('data-task-id');
    const cid = await page.locator('#conversation-list .conv-item').first().getAttribute('data-cid');
    expect(await oldRow.locator('.chat-queue-actions button').evaluateAll((buttons) => buttons.map((button) => button.getAttribute('data-act'))))
    ...
    app.setModelMode('success');
    app.setModelTextReplies(['Updated draft delivered.']);
    app.releaseControlledModelChunk();
    app.finishControlledModelStream();
    await expect.poll(async () => (await app.invoke<{ tasks: Array<{ status: string }> }>('groupChat.tasks.list', { cid }))
      .tasks.filter((task) => task.status === 'running').length).toBe(0);
    await expect(oldRow).toBeVisible();
    ...
    expect(request).toContain('Updated queued wording.');
    expect(request).toContain('Keep this second line.');
    expect(request).not.toContain('old pending wording.');
    const tasks = await app.invoke<{ tasks: Array<{ task_id: string; instruction: string }> }>('groupChat.tasks.list', { cid });
    expect(tasks.tasks.find((task) => task.task_id === taskId)?.instruction).toBe(edited);
    await expect(page.locator('#chat-history')).toContainText('Updated queued wording.');
    await expect(page.locator('#chat-send-btn')).not.toHaveClass(/\bstreaming\b/, { timeout: 20_000 });
  });

  test('cancels the editing message from the queue after the preceding reply finishes', async ({ modelOrkas, boardPage }) => {
    const app = modelOrkas;
    const page = boardPage;
    const input = page.locator('#chat-input');
    const before = app.modelRequests.length;
    app.setModelMode('controlled-slow');
    await input.fill('@ContentWriter finish the preceding reply.');
    await input.press('Enter');
    await expect.poll(() => app.modelRequests.length).toBe(before + 1);
    await input.fill('@ContentWriter cancel this queued message.');
    await input.press('Enter');
    const row = page.locator('#chat-task-board-list .chat-queue-item', { hasText: 'cancel this queued message.' });
    ...
    expect(tasks.tasks.find((task) => task.task_id === taskId)?.status).toBe('cancelled');
    await input.fill('@ContentWriter continue after cancellation.');
    await input.press('Enter');
    await expect.poll(() => app.modelRequests.length).toBe(before + 2);
    const requests = JSON.stringify(app.modelRequests.slice(before));
    expect(requests).not.toContain('cancel this queued message.');
    expect(requests).not.toContain('Never send this unsaved revision');
    await expect(page.locator('#chat-send-btn')).not.toHaveClass(/\bstreaming\b/);
  });

  test('retains composer edits across reload and failed save, then cancels without reviving the message', async ({ modelOrkas, boardPage }) => {
    const app = modelOrkas;
    const page = boardPage;
    const before = app.modelRequests.length;
    const input = page.locator('#chat-input');
    app.setModelMode('controlled-slow');
    await input.fill('@ContentWriter keep working on the draft.');
    await input.press('Enter');
    await expect.poll(() => app.modelRequests.length).toBe(before + 1);
    await input.fill('@ContentWriter original queued instruction.');
    await input.press('Enter');
    const row = page.locator('#chat-task-board-list .chat-queue-item', { hasText: 'original queued instruction.' });
    await expect(row).toBeVisible();
    const taskId = await row.getAttribute('data-task-id');
    const cid = await page.locator('#conversation-list .conv-item').first().getAttribute('data-cid');
    ...
    expect(tasks.tasks.find((task) => task.task_id === taskId)?.status).toBe('cancelled');
    expect(app.modelRequests).toHaveLength(before + 1);
    await expect(page.locator('#chat-history')).not.toContainText('My unsaved changes');
  });
